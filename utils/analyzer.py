...
This is intentionally lightweight so you can explain the logic in interviews.
"""
import re
from collections import Counter

# small curated skill list; expand later as you learn
KNOWN_SKILLS = [
...
def top_words(text: str, n=10):
    # return common non-stop words for a quick insight
    stops = {
        "the","and","to","of","in","for","with","a","an","on","is","as","by","that"
    }
...
def summarize(text: str, max_chars=200):
    # extremely simple first-sentence summary fallback
...
def analyze_resume(text: str):
    ...
    # a simple scoring heuristic (explainable in interview)
    score = 50
    score += min(len(skills) * 5, 25)           # skill bonus
    score += 5 if words > 300 else 0            # length bonus
    score = min(score, 100)
